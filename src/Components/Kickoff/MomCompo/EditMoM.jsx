...
const EditMoM = ({ onClose, onUpdate, momId }) => {
  ...
  const fetchMom = async () => {
    try {
      const response = await axiosInstance.get(
        `/kickoff/getSingleMomById/${momId}`
      );
      ...
      if (momInfo.customerName)
        setSelectedCustomer({
          label: momInfo.customerName,
          value: momInfo.customerId,
        });
      if (momInfo.projectName)
        setSelectedProject({
          label: momInfo.projectName,
          value: momInfo.projectId,
        });
      if (momInfo.itemNo)
        setSelectedItem({ label: momInfo.itemNo, value: momInfo.itemNo });

      if (momInfo.contactPersonName) {
        setContactPerson(
          momInfo.contactPersonName
            .split(",")
            .map((name) => ({ label: name.trim(), value: name.trim() }))
        );
      }
      if (momInfo.employeeName && momInfo.employeeeId) {
        const employeeNames = momInfo.employeeName
          .split(",")
          .map((name) => name.trim());
        const employeeIds = momInfo.employeeeId
          .split(",")
          .map((id) => id.trim());
        setEmplyees(
      ...
      if (momInfo.thirdPersonCompany) {
        const personsArray = momInfo.thirdPersonCompany
          .split(",")
          .map((name) => name.trim())
          .filter(Boolean);
        setThirdCompanyPerson(
          personsArray.map((name) => ({ value: name, label: name }))
        );
      } else {
        setThirdCompanyPerson([]);
      }
  ...
  const handleRemoveSection = async (sectionId) => {
    ...
    const payload = {
      ...
      momEntryIds: sectionToRemove.entries
        .map((entry) => entry.momEntryId)
        .filter(Boolean),
    };
    ...
    if (payload.momEntryIds.length > 0) {
      try {
        setIsSaving(true);
        await axiosInstance.delete("/kickoff/deleteBulkEntries", {
          data: payload,
        });
        toast.success(
          `Section for WO# ${payload.workOrderNo} has been marked for deletion.`
        );
      } catch (error) {
    ...
    setMomSections((prevSections) =>
      prevSections.filter((sec) => sec.id !== sectionId)
    );
  };
    ...
    if (entryToRemove.momEntryId) {
      try {
        ...
        await axiosInstance.delete(
          `/kickoff/deleteSingleMOMEntries/${entryToRemove.momEntryId}`
        );
    ...
    setMomSections((prevSections) =>
      prevSections.map((sec) => {
        if (sec.id === sectionId) {
          ...
          const updatedEntries = sec.entries.filter(
  ...
  const handleImageRemove = async (
    ...
    if (imageToRemove.imageId) {
      try {
        ...
        await axiosInstance.delete(
          `/kickoff/deleteMOMImage/${imageToRemove.imageId}`
        );
      ...
      prevSections.map((sec) => {
        if (sec.id === sectionId) {
          const updatedEntries = sec.entries.map((ent) => {
            if (ent.id === entryId) {
              const updatedImages = ent[fieldName].filter(
  ...
  const handleUpdate = async () => {
    ...
    try {
      ...
      const momInfoPayload = {
        ...
        thirdPersonCompany: thirdCompanyPerson
      ...
      const updateInfoPromise = axiosInstance.put(
        "/kickoff/updateMOMInfo",
        momInfoPayload
      );
      const updateEntriesPromise = axiosInstance.put(
        "/kickoff/updateMOMEntry",
        momEntriesPayload
      );
        ...
        <Container fluid>
          ...
          <Row>
            ...
            <Col md={6}>
              <div className="form-group">
                <label>
                  Venue <span className="required-label">*</span>
                </label>
                <input
                  type="text"
          ...
          <Row>
            <Col md={6}>
              <div className="form-group">
                <label>
                  Contact Persons <span className="required-label">*</span>
                </label>
                <Select
                  ...
                  value={contactPerson}
                  onChange={(selectedOption) =>
                    setContactPerson(selectedOption)
                  }
                />
              </div>
            </Col>
            <Col md={6}>
              <div className="form-group">
                <label>
                  Project <span className="required-label">*</span>
                </label>
                <Select
          ...
          <Row>
            <Col md={6}>
              <div className="form-group">
                <label>
                  Employee <span className="required-label">*</span>
                </label>
                <Select
            ...
            <Col md={6}>
              <div className="form-group">
                <label>
                  Item no <span className="required-label">*</span>
                </label>
                <Select
                  options={itemOptions}
                  value={selectedItem}
          ...
          <div key={section.id} className="mec-container mt-4">
              ...
              <div className="mec-form-group">
                ...
                <Select
                  options={workOrderNumberOptions.map((option) => ({
                    ...option,
                    isDisabled:
                      selectedWoValues.includes(option.value) &&
                      option.value !== section.workOrderNumber?.value,
                  }))}
            ...
            <div className="mec-entries-section">
              ...
              <div className="mec-grid mec-grid--header">
                ...
                <div className="mec-grid-cell mec-grid-cell--header">
                  Observation *
                </div>
                <div className="mec-grid-cell mec-grid-cell--header">
                  Detail/Action Plan
              ...
              {section.entries.map((entry, entryIndex) => (
                <div key={entry.id} className="mec-grid mec-grid--entry-row">
                  <div className="mec-grid-cell mec-grid-cell--index">
                    {entryIndex + 1}
                  </div>
                  <div className="mec-grid-cell">
                    <textarea
                      ...
                      onChange={(e) =>
                        handleInputChange(section.id, entry.id, e)
                      }
                    />
                  </div>
                  <div className="mec-grid-cell">
                    <textarea
                      name="actionPlan"
                      placeholder="Enter detail or action plan"
                    ...
                    <div className="mec-image-upload-container">
                      {entry.illustrations.map((image, idx) => (
                        <div key={idx} className="mec-image-preview">
                          ...
                          <button
                            type="button"
                            className="mec-remove-image-btn"
                            onClick={() =>
                              handleImageRemove(
                                section.id,
                                entry.id,
                  ...
                  <div className="mec-grid-cell">
                    <div className="mec-image-upload-container">
                      ...
                      <label className="mec-upload-button">
                        ...
                        />
                        <span className="mec-upload-icon">+</span>
                        {entry.correctedImages.length > 0
                          ? "Add More"
                          : "Corrected"}
                      </label>
                    </div>
                  ...
                  <div className="mec-grid-cell">
                    <textarea
                      ...
                      placeholder="Enter corrected points"
                      value={entry.correctedPoints}
                      onChange={(e) =>
                        handleInputChange(section.id, entry.id, e)
                      }
                    />
                  ...
                  <div className="mec-grid-cell">
                    <textarea
                      name="responsibleTarget"
                      placeholder="Enter responsible person and target"
                      value={entry.responsibleTarget}
                      onChange={(e) =>
                        handleInputChange(section.id, entry.id, e)
                      }
                    />
