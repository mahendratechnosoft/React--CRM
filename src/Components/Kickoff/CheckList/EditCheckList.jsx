...
import "../../../CompanyComponent/ChecklistSheet/CreateChecklistSheet.css"; 
import Select from "react-select";
import axiosInstance from "../../../BaseComponet/axiosInstance";
import { toast } from "react-toastify";

const EditCheckList = ({ onClose, onUpdate, checkListId }) => {
  ...
  const fetchAndPopulateChecklistData = async () => {
    ...
    try {
      ...
      const response = await axiosInstance.get(
        `/kickoff/getCheckListData/${checkListId}`
      );
      ...
      setPocketMCReleaseDate(
        checkListInfo.pocketMCReleaseDate?.split("T")[0] || ""
      );
      setFinalMCReleaseDate(
        checkListInfo.finalMCReleaseDate?.split("T")[0] || ""
      );
      ...
      setSelectedCustomer({
        value: checkListInfo.customerId,
        label: checkListInfo.customerName,
      });

      await fetchProjects(checkListInfo.customerId, checkListInfo.projectName);
      setSelectedProject({
        value: checkListInfo.projectId,
        label: checkListInfo.projectName,
      });

      await fetchDesigners(checkListInfo.projectId, checkListInfo.designerName);
      setSelectedDesigner({
        value: checkListInfo.employeeId,
        label: checkListInfo.designerName,
      });

      await fetchWoNo(checkListInfo.employeeId, checkListInfo.workOrderNumber);
      setSelectedWoNo({
        value: checkListInfo.workOrderNumber,
        label: checkListInfo.workOrderNumber,
      });
  ...
  const fetchChecklistTemplateAndSetSelections = async (savedItems) => {
    try {
      ...
      const response = await axiosInstance.get(
        "/kickoff/getCheckListCategoryAndItem"
      );
        ...
        category.items.forEach((templateItem) => {
          ...
          if (savedItem) {
            newSelections[templateItem.itemId] = {
              designerCheck:
                checkOptions.find(
                  (opt) => opt.value === savedItem.checkByDesigner
                ) || null,
              tlCheck:
                checkOptions.find(
                  (opt) => opt.value === savedItem.checkByTeamLead
                ) || null,
            };
          } else {
            newSelections[templateItem.itemId] = {
  ...
  const fetchWoNo = async (
    designerId = selectedDesigner?.value,
    ...
    try {
      const response = await axiosInstance.get(
        `/kickoff/getWorkOrderByEmployeeId/${designerId}`
      );
      const options = response.data.map((wo) => ({
      ...
      checkListItems: checklistData.flatMap((category) =>
        category.items.map((item) => {
          ...
          return {
            ...
            checkByDesigner:
              checklistSelections[item.itemId]?.designerCheck?.value || "N/A",
            checkByTeamLead:
              checklistSelections[item.itemId]?.tlCheck?.value || "N/A",
          };
        })
      ),
    };
      ...
      <div className="main-container">
        ...
        <Container fluid>
          <Row className="mb-4">
            ...
            <Col>
              <label htmlFor="woNo" className="form-label">
                <span className="required-label">*</span>WO No
              </label>
              <Select
                options={woNoOptions}
                value={selectedWoNo}
            ...
            <Col>
              <label htmlFor="revNo" className="form-label">
                Rev No.
              </label>
              <input
                type="text"
          ...
          <Row className="mb-4">
            ...
            <Col md={6}>
              <label htmlFor="toolDetails" className="form-label">
                Tool Details
              </label>
              <textarea
                id="toolDetails"
          ...
          <Row className="mb-3">
            ...
            <Col md={3}>
              <label htmlFor="designEndDate" className="form-label">
                Design End Date
              </label>
              <input
                type="date"
            ...
            <Col md={3}>
              <label htmlFor="pocketMCReleaseDate" className="form-label">
                Pocket MC Release Date
              </label>
              <input
                type="date"
                id="pocketMCReleaseDate"
                className="form-control"
            ...
            <Col md={3}>
              <label htmlFor="finalMCReleaseDate" className="form-label">
                Final MC Release Date
              </label>
              <input
                type="date"
                id="finalMCReleaseDate"
                className="form-control"
                  ...
                  <tr className="item-row" key={item.itemId}>
                    ...
                    <td>
                      <Select
                        ...
                        value={
                          checklistSelections[item.itemId]?.designerCheck ||
                          null
                        }
                        onChange={(selectedOption) =>
                          handleDesignerCheckChange(item.itemId, selectedOption)
                        }
                        placeholder="Select..."
                        isClearable
                      />
                    ...
                    <td>
                      <Select
                        options={checkOptions}
                        value={
                          checklistSelections[item.itemId]?.tlCheck || null
                        }
                        onChange={(selectedOption) =>
                          handleTLCheckChange(item.itemId, selectedOption)
                        }
                        placeholder="Select..."
                        isClearable
                      />
