...
const QuotationList = () => {
  ...
  const [selectedQuotationIdForPdf, setSelectedQuotationIdForPdf] =
    useState(null);
  ...
  useEffect(() => {
    ...
    const roleData = localStorage.getItem("role"); // <-- separate key
  ...
  const hasAccess = (permissionKey) => {
    ...
    return true; // non-ROLE_EMP → always allow
  };
  ...
  const fetchQuotations = async () => {
    ...
    try {
      ...
      const response = await axiosInstance.get(
        `/sales/getAllQuotation/${page}/${size}?companyName=${companyName}`
      );
