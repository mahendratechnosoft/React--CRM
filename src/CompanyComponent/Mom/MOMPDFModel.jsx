    ...
    <Document>
      <Page size="A4" orientation="landscape" style={MOMPdfStyles.page}>
        ...
        <View style={MOMPdfStyles.row}>
          <Text style={MOMPdfStyles.titleCellHeading}>
            {" "}
            MOM-{workOrderInfo.partNumber}_{workOrderInfo.partName} Dies DAP MOM
            SHEET
          </Text>
        </View>
        <View style={MOMPdfStyles.row}>
          <Text style={MOMPdfStyles.titleCellLeft}>
            {" "}
            Project :{momInfo.projectName}
          </Text>
          <Text style={MOMPdfStyles.titleCell}> Venue : {momInfo.venue}</Text>
          <Text style={MOMPdfStyles.titleCell}>
            {" "}
            Date : {momInfo.createdDate}{" "}
          </Text>
        </View>
        <View style={MOMPdfStyles.row}>
        ...
        <View style={MOMPdfStyles.row}>
          ...
          <View style={MOMPdfStyles.column}>
            {momInfo.contactPersonName.split(",").map((_, index) => (
              <Text key={index} style={MOMPdfStyles.cellText}>
                {index + 1}
              </Text>
            ))}
          </View>
          ...
          <View style={MOMPdfStyles.column}>
            {momInfo.employeeName.split(",").map((name, index) => (
              <Text key={index} style={MOMPdfStyles.lastNamecolumn}>
                {name.trim()}
              </Text>
            ))}
          </View>
        ...
        {momInfo.thirdPersonCompany.split(",").map((name, index) => (
          <View style={MOMPdfStyles.row} key={index}>
            ...
            <Text style={MOMPdfStyles.thidPartyCustomerName}>
              {name.trim()}
            </Text>
          </View>
        ))}
        <View style={MOMPdfStyles.row}>
          <Text style={MOMPdfStyles.intoducntionCell}>
            {" "}
            Introduction :
            <Text style={MOMPdfStyles.introductionText}>
              {momInfo.introduction}
            </Text>
          </Text>
        </View>
        Mom entries
        <View style={MOMPdfStyles.row}>
          ...
          <Text style={MOMPdfStyles.momEntriesHeader}>
            Responsible & Target
          </Text>
        </View>
        {Object.keys(groupedEntries).map((workOrderNo) => {
          ...
          return (
            <View key={workOrderNo}>
              ...
              <View style={MOMPdfStyles.row}>
                <Text style={MOMPdfStyles.workOrderHeader}>
                  OP-{processInfo.operationNumber}(
                  {processInfo.process || "N/A"})
                </Text>
              </View>
              ...
              {groupedEntries[workOrderNo].map((entry, index) => (
                <View
                  style={MOMPdfStyles.row}
                  key={entry.momEntryId || `${workOrderNo}-${index}`}
                >
                  <Text style={MOMPdfStyles.cell}>{index + 1}</Text>
                  <Text style={MOMPdfStyles.cell}>
                    {entry.observation || ""}
                  </Text>
                  ...
                  <Text style={MOMPdfStyles.cell}>
                    {entry.correctedPoints || ""}
                  </Text>
                  <Text style={MOMPdfStyles.cell}>
                    {entry.responsibleAndTarget || ""}
                  </Text>
                </View>
              ))}
            </View>
...
export default function MOMPDFModel({ show, onClose, momId }) {
  ...
  const fetchMomData = async () => {
    ...
    try {
      ...
      const workOrderResponse = await axiosInstance.get(
        `/work/geSingletWorkOrderItemNo/${response.data.momInfo.itemNo}`
      );
      setWorkOrder(workOrderResponse.data);

      const workOrderItemProcessResponse = await axiosInstance.get(
        `/work/getWorkOrderById/${workOrderResponse.data.workOrderId}`
      );
      setWorkOrderItemProccess(
        workOrderItemProcessResponse.data.workOrderItems
      );
    } catch (error) {
      console.error(error);
    } finally {
